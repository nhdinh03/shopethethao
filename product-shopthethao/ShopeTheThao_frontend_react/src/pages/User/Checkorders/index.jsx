...
import { 
  FaSearch, FaBox, FaTruck, FaCheckCircle, FaTimesCircle, FaAngleDown, FaAngleUp, 
  FaTimes, FaEye, FaCalendarAlt, FaFilter, FaSortAmountDown, FaSortAmountUp,
  FaMapMarkerAlt, FaCreditCard, FaStickyNote, FaSpinner, FaEraser, FaRegClock,
  FaMoneyBillWave, FaTags, FaThList, FaRegBell
} from 'react-icons/fa';
...
function Checkorders() {
  ...
  const [recentSearches, setRecentSearches] = useState(['Áo Arsenal', 'Giày Nike', 'Adidas']);
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      // Thực tế sẽ gọi API đến server của bạn
      // const response = await axios.get(API_URL);
      
      // Dữ liệu mẫu từ API mà bạn đã cung cấp
      const sampleOrderItems = [
        {
          ...
          product: {
            ...
            images: [
              {
                id: 808,
                imageUrl: "422a1a02-523e-4650-9516-bfdea3ebb972_Áo tập luyện chính hãng Arsenal 2024 25 2.jpg"
              }
            ]
          },
          size: {
      ...
      // Tạo mẫu đơn hàng với các trạng thái khác nhau và thời gian đa dạng
      const sampleOrders = [
        {
          id: 1,
          orderCode: "#10",
          ...
          customerCode: 'U10',
          trackingSteps: [
            { status: 'ordered', time: '20/02/2025 00:11', title: 'Đơn hàng đã đặt' },
            { status: 'processing', time: '20/02/2025 08:30', title: 'Đang xử lý' },
            { status: 'cancelled', time: '21/02/2025 10:15', title: 'Đã hủy' }
          ]
        },
        {
          id: 2,
          ...
          customerCode: 'U11',
          trackingSteps: [
            { status: 'ordered', time: '15/02/2025 08:30', title: 'Đơn hàng đã đặt' },
            { status: 'processing', time: '15/02/2025 10:45', title: 'Đang xử lý' },
            { status: 'shipping', time: '16/02/2025 09:20', title: 'Đang giao hàng' },
            { status: 'delivered', time: '17/02/2025 14:30', title: 'Đã nhận hàng' }
          ]
        },
        {
          id: 3,
          ...
          customerCode: 'U12',
          trackingSteps: [
            { status: 'ordered', time: '18/02/2025 15:22', title: 'Đơn hàng đã đặt' },
            { status: 'processing', time: '18/02/2025 16:45', title: 'Đang xử lý' },
            { status: 'shipping', time: '19/02/2025 08:30', title: 'Đang giao hàng' }
          ]
        },
        {
          id: 4,
          ...
          customerCode: 'U13',
          trackingSteps: [
            { status: 'ordered', time: '01/01/2025 11:30', title: 'Đơn hàng đã đặt' },
            { status: 'processing', time: '01/01/2025 14:45', title: 'Đang xử lý' },
            { status: 'shipping', time: '02/01/2025 09:20', title: 'Đang giao hàng' }
          ]
        }
      ];
  ...
  const getStatusIcon = (status) => {
    switch(status) {
      case 'pending': 
      case 'shipping': return <FaTruck className="status-icon pending" />;
      ...
      case 'ordered': return <FaBox className="status-icon ordered" />;
  ...
  const generateSuggestions = (query) => {
    const allSuggestions = [
      'Áo bóng đá Arsenal', 'Áo bóng đá Chelsea', 'Áo bóng đá Manchester United',
      'Giày Nike Mercurial', 'Giày Adidas Predator', 'Giày Puma Future',
      'Bộ quần áo thể thao', 'Áo thun thể thao', 'Quần short thể thao'
    ];
    ...
    <div className="check-orders">
          ...
          <form onSubmit={handleSearch} className="search-form">
            <div className={`search-input ${searchFocused ? 'focused' : ''}`}>
              ...
              <input 
                ...
                onFocus={() => {setSearchFocused(true); setSearchHistory(true);}}
            ...
            {searchHistory && (
              <div className="search-history" ref={searchHistoryRef}>
                {recentSearches.length > 0 && (
                  <div className="search-section">
                    <div className="section-header">
                      <h4><FaRegClock /> Tìm kiếm gần đây</h4>
                    ...
                    <ul>
                      {recentSearches.map((term, idx) => (
                        <li key={`recent-${idx}`} onClick={() => applySearchTerm(term)}>
                ...
                {searchSuggestions.length > 0 && (
                  <div className="search-section">
                    <div className="section-header">
                      <h4><FaTags /> Gợi ý tìm kiếm</h4>
                    </div>
                    <ul>
                      {searchSuggestions.map((suggestion, idx) => (
                        <li key={`suggestion-${idx}`} onClick={() => applySearchTerm(suggestion)}>
                ...
                <div className="search-section">
                  <div className="section-header">
                    <h4><FaFilter /> Lọc nâng cao</h4>
                  </div>
                  <div className="advanced-search-options">
                    <div className="search-option" onClick={() => setShowPriceFilter(!showPriceFilter)}>
      ...
      {showFilters && (
        <div className="advanced-filters">
          <div className="filter-group date-range">
            <div className="filter-label"><FaCalendarAlt /> Khoảng thời gian:</div>
      ...
      <div className="orders-list">
        {isEmptyResults ? (
          ...
          Object.entries(filteredOrdersGroups).map(([monthYear, monthOrders]) => (
            <div key={monthYear} className="orders-month-group">
              ...
              <div className="month-orders">
                {monthOrders.map(order => (
                  <div key={order.id} className={`order-item ${expandedOrderId === order.id ? 'expanded' : ''}`}>
                    <div className="order-header" onClick={() => toggleOrderDetails(order.id)}>
                      <div className="order-basic-info">
                        <span className="order-id">Đơn hàng {order.orderCode}</span>
                      ...
                      <div className="order-status-container">
                        ...
                        {expandedOrderId === order.id ? <FaAngleUp className="toggle-icon" /> : <FaAngleDown className="toggle-icon" />}
                      </div>
                    </div>
                    ...
                    <div className="order-content">
                        ...
                        <div key={index} className="order-product">
                          <img 
                            src={item.product.images && item.product.images.length > 0 
                              ? getProductImageUrl(item.product.images[0].imageUrl) 
                          ...
                          <div className="product-info">
                            ...
                            <div className="product-price-qty">
                              <span className="product-price">{item.unitPrice.toLocaleString()}đ</span>
                              <span className="product-size-qty">Size: {item.size.name} | x{item.quantity}</span>
                            </div>
                          </div>
                        </div>
                      ))}
                      ...
                      <button className="view-details-btn" onClick={(e) => {e.stopPropagation(); openOrderDetails(order);}}>
                    ...
                    {expandedOrderId === order.id && (
                      <div className="order-details">
                        ...
                        <div className="details-row total-row">
                          <span className="details-label">Tổng tiền đơn hàng:</span>
                          <span className="details-value total-value">{order.total.toLocaleString()}đ</span>
                        </div>
                    ...
                    <div className="order-footer">
                      <span className="order-total">Tổng tiền: {order.total.toLocaleString()}đ</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="modal-body">
                ...
                <div className="tracking-timeline">
                  {selectedOrder.trackingSteps.map((step, index) => (
                    <div key={index} className={`tracking-step ${step.status}`}>
                      ...
                      {index < selectedOrder.trackingSteps.length - 1 && <div className="step-connector"></div>}
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="info-section">
                ...
                <div className="info-row">
                  ...
                  <span className="info-value">{selectedOrder.customerCode}</span>
                </div>
                <div className="info-row">
              ...
              <div className="modal-actions">
                ...
                <button className="contact-btn">Liên hệ shop</button>
                <button className="close-modal-btn" onClick={closeOrderModal}>Đóng</button>
              </div>
            </div>
          </div>
        </div>
